...
$sql = 'SELECT poliza_estado_id, cliente_nombre, cliente_email, seguro_email_emision, seguro_email_fotos, seguro_email_rastreador, subtipo_poliza_id, patente, IF(COUNT(poliza_foto_id) > 0, 1, 0) as poliza_fotos, equipo_rastreo, equipo_rastreo_pedido_nombre FROM poliza JOIN (cliente, productor_seguro, seguro) ON poliza.cliente_id = cliente.cliente_id AND poliza.productor_seguro_id = productor_seguro.productor_seguro_id AND productor_seguro.seguro_id = seguro.seguro_id LEFT JOIN (automotor, equipo_rastreo_pedido) ON poliza.poliza_id = automotor.poliza_id AND automotor.equipo_rastreo_pedido_id = equipo_rastreo_pedido.equipo_rastreo_pedido_id LEFT JOIN poliza_foto ON poliza.poliza_id = poliza_foto.poliza_id WHERE poliza.poliza_id='.$poliza_id;
$res = mysql_query($sql);
list($state, $cliente_nombre, $cliente_email, $seguro_email_emision, $seguro_email_fotos, $seguro_email_rastreador, $subtipo_poliza_id, $patente, $fotos, $equipo_rastreo, $equipo_rastreo_pedido) = mysql_fetch_array($res);
?>
        ...
        <fieldset class="ui-widget ui-widget-content ui-corner-all">
			...
			<p>
				<div id="doc">
					...
					<input type="radio" id="doc2" name="type" value="pe" mail="<?=$seguro_email_emision?>" subject='EMITIR <?=$patente?>' /><label for="doc2">Pedido de Emisión</label>
					<input type="radio" id="doc3" name="type" value="pemc" mail="<?=$seguro_email_emision?>" subject='M/C <?=$patente?>' /><label for="doc3">Pedido de M/C</label>
...
<script>
$('#doc1, #doc2, #doc3, #doc4, #doc5, #doc6').change(function() {
