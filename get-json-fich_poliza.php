...
<?php
	...
	$query_Recordset1 = sprintf("SELECT poliza.poliza_id, sucursal_nombre, sucursal.sucursal_id, TRIM(CONCAT(IFNULL(cliente_apellido, ''), ' ', IFNULL(cliente_nombre, ''))) as cliente_nombre, tipo_poliza_nombre, subtipo_poliza_nombre, IF(COUNT(endoso_id) > 0, 'ANULADA', poliza_estado_nombre) as poliza_estado_nombre, poliza_numero, poliza_renueva_num, productor_seguro.seguro_id, productor_seguro.productor_seguro_id, CONCAT(poliza_vigencia, IF(poliza_vigencia='Otra', CONCAT(' (', poliza_vigencia_dias, ')'), '')) as poliza_vigencia, poliza_validez_desde, poliza_validez_hasta, poliza_cuotas, poliza_cant_cuotas, poliza_fecha_solicitud, poliza_fecha_emision, poliza_fecha_recepcion, poliza_fecha_entrega, poliza_correo, poliza_email, poliza_entregada, poliza_prima, poliza_premio, poliza_medio_pago, poliza_pago_detalle, poliza_recargo, poliza_descuento, SUM(cuota_pfc) as cuota_pfc
