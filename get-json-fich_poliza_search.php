...
<?php
	// Main Query
	$query_Recordset1 = "SELECT poliza.poliza_id, automotor_id, TRIM(CONCAT(IFNULL(cliente_nombre, ''), ' ', IFNULL(cliente_apellido, ''))) as cliente_nombre, poliza_numero, CONCAT(DATE_FORMAT(poliza_validez_desde, '%d/%m/%y'), ' - ', DATE_FORMAT(poliza_validez_hasta, '%d/%m/%y')) AS validez, seguro_nombre, productor_seguro_codigo, CONCAT(IF(automotor_carroceria_id=17, '101', ''), patente_0, patente_1) as patente, IF(COUNT(endoso_id)>0, 2, IF(poliza_estado_id IN (3,4), 1, 0)) as highlight, CONCAT(patente_0, patente_1) as patente, cliente.cliente_id FROM poliza JOIN (subtipo_poliza, productor_seguro, seguro, cliente) ON poliza.subtipo_poliza_id = subtipo_poliza.subtipo_poliza_id AND poliza.productor_seguro_id = productor_seguro.productor_seguro_id AND productor_seguro.seguro_id = seguro.seguro_id AND cliente.cliente_id = poliza.cliente_id LEFT JOIN automotor ON poliza.poliza_id = automotor.poliza_id LEFT JOIN (endoso, endoso_tipo) ON (poliza.poliza_id = endoso.poliza_id AND endoso.endoso_tipo_id = endoso_tipo.endoso_tipo_id AND endoso_tipo_grupo_id = 1)";
