...
<?php
	// Main Query
	$query_Recordset1 = "SELECT poliza_id, cliente_nombre, poliza_numero, CONCAT(DATE_FORMAT(poliza_validez_desde, '%d/%m/%y'), ' - ', DATE_FORMAT(poliza_validez_hasta, '%d/%m/%y')) AS validez, seguro_nombre FROM poliza JOIN (subtipo_poliza, productor_seguro, seguro, cliente) ON poliza.subtipo_poliza_id = subtipo_poliza.subtipo_poliza_id AND poliza.productor_seguro_id = productor_seguro.productor_seguro_id AND productor_seguro.seguro_id = seguro.seguro_id AND cliente.cliente_id = poliza.cliente_id WHERE 1";
	...
	if (isset($_GET['box0-poliza_numero']) or isset($_GET['box0-cliente_nombre'])) {
		...
		if (isset($_GET['box0-cliente_nombre']) && $_GET['box0-cliente_nombre'] !== '') {
			$query_Recordset1 .= sprintf(" AND cliente_nombre LIKE %s",
