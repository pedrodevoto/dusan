	...
	switch($row_Recordset1['subtipo_poliza_tabla']) {
		...
		case 'incendio_edificio':
			...
			case 'integral_consorcio':
				...
				break;
			case 'otros_riesgos':
				// Recordset: Otros Riesgos
				$query_Recordset2 = sprintf("SELECT *  FROM otros_riesgos WHERE poliza_id=%s", $row_Recordset1['poliza_id']);
				$Recordset2 = mysql_query($query_Recordset2, $connection) or die(mysql_die());
				$row_Recordset2 = mysql_fetch_assoc($Recordset2);
				$totalRows_Recordset2 = mysql_num_rows($Recordset2);
				...
				switch(substr($_GET['type'], 0, 2)) {
					case 'cc':
						...
						*****************************************/
						...
						$pdf->SetXY($x + 2, $y);
						$pdf->SetFont('Arial', '', 8);
						$pdf->Write(5, 'Riesgo asegurado: '.trimText($row_Recordset2['otros_riesgos_riesgo'], $pdf, 60));
			
						$y +=5;
						$pdf->SetXY($x + 2, $y);
						$pdf->Write(5, 'Datos del riesgo: '.$row_Recordset2['otros_riesgos_datos_riesgo']);

						$y +=5;
						$pdf->SetXY($x + 2, $y);
						$pdf->Write(5, 'Detalle del riesgo: '.$row_Recordset2['otros_riesgos_detalle_riesgo']);				
