...
<?php
	...
	switch($row_Recordset1['subtipo_poliza_tabla']) {
		...
		case 'integral_comercio':
			...
			// Recordset: Combinado Familiar
			$query_Recordset2 = sprintf("SELECT *  FROM integral_comercio WHERE poliza_id=%s", $row_Recordset1['poliza_id']);
			$Recordset2 = mysql_query($query_Recordset2, $connection) or die(mysql_die());
			$row_Recordset2 = mysql_fetch_assoc($Recordset2);
			$totalRows_Recordset2 = mysql_num_rows($Recordset2);
			...
			$objects = array(
				array('db'=>'bienes_de_uso', 'desc'=>'Bienes de Uso')
			);

			for ($i = 0; $i < count($objects); $i++) {
				$objects[$i]['items'] = array();
				$query_Recordset3 = sprintf('SELECT integral_comercio_%1$s_cantidad as cantidad, integral_comercio_%1$s_producto as producto, integral_comercio_%1$s_marca as marca, integral_comercio_%1$s_serial as serial_no, integral_comercio_%1$s_valor as valor FROM integral_comercio_%1$s WHERE integral_comercio_id=%2$s', 
					$objects[$i]['db'], 
					$row_Recordset2['integral_comercio_id']);
			...
			switch(substr($_GET['type'], 0, 2)) {
				case 'cc':
					...
					*****************************************/
					...
					$pdf->SetXY($x + 2, $y);
					$pdf->SetFont('Arial', '', 8);
					$pdf->Write(5, 'Direccion: '.trimText($row_Recordset2['integral_comercio_domicilio_calle'], $pdf, 70).' '.$row_Recordset2['integral_comercio_domicilio_nro']);
					$pdf->SetX($x + 65 + 30);
					$pdf->Write(5, 'Piso/Dpto: '.$row_Recordset2['integral_comercio_domicilio_piso'].' '.$row_Recordset2['integral_comercio_domicilio_dpto']);

					$pdf->SetX($x + 95 + 30);
					$pdf->Write(5, 'Localidad: '.trimText($row_Recordset2['integral_comercio_domicilio_localidad'], $pdf, 60));
					$pdf->SetX($x + 180);
					$pdf->Write(5, 'CP: '.trimText($row_Recordset2['integral_comercio_domicilio_cp'], $pdf, 60));


					$y +=5;
					$pdf->SetXY($x + 2, $y);
					$pdf->Write(5, 'Actividad: '.$row_Recordset2['integral_comercio_actividad']);
					$pdf->SetX($x + 95);
					$pdf->Write(5, 'Lote: '.'LOTE');

	
					$y +=5;
					$pdf->SetXY($x + 2, $y);
					$pdf->Write(5, 'Valor tasado de la propiedad: $'.formatNumber($row_Recordset2['integral_comercio_valor_tasado'], 2));
					...
					$pdf->SetXY($x + 2, $y);
					$pdf->SetFont('Arial', '', 8);
					$pdf->Write(5, 'Incendio Contenido: $'.formatNumber($row_Recordset2['integral_comercio_inc_contenido'], 2));
					$pdf->SetX($x + 65 + 30);
					$pdf->Write(5, 'Robo Matafuegos: $'.formatNumber($row_Recordset2['integral_comercio_robo_matafuegos'], 2));

					$y += 5;
					$pdf->SetXY($x + 2, $y);
					$pdf->SetFont('Arial', '', 8);
					$pdf->Write(5, 'Robo a Primer Riesgo Absoluto: $'.formatNumber($row_Recordset2['integral_comercio_robo_pra'], 2));
					$pdf->SetX($x + 65 + 30);
					$pdf->Write(5, trimText('Robo Luces, Cámaras, Mangueras: $', $pdf, 120).formatNumber($row_Recordset2['integral_comercio_robo_lcm'], 2));

					$y += 5;
					$pdf->SetXY($x + 2, $y);
					$pdf->SetFont('Arial', '', 8);
					$pdf->Write(5, 'Cristales a Primer Riesgo Absoluto: $'.formatNumber($row_Recordset2['integral_comercio_cristales_pra'], 2));
					$pdf->SetX($x + 65 + 30);
					$pdf->Write(5, trimText('Daños por Agua: $', $pdf, 120).formatNumber($row_Recordset2['integral_comercio_danios_agua'], 2));

					$y += 5;
					$pdf->SetXY($x + 2, $y);
					$pdf->SetFont('Arial', '', 8);
					$pdf->Write(5, 'RC Comprensiva: $'.formatNumber($row_Recordset2['integral_comercio_rc_comprensiva'], 2));
					$pdf->SetX($x + 65 + 30);
					$pdf->Write(5, 'RC Garage: $'.formatNumber($row_Recordset2['integral_comercio_rc_garage'], 2));

					$y += 5;
					$pdf->SetXY($x + 2, $y);
					$pdf->SetFont('Arial', '', 8);
					$pdf->Write(5, 'RC Ascensor: $'.formatNumber($row_Recordset2['integral_comercio_rc_ascensor'], 2));
					$pdf->SetX($x + 65 + 30);
					$pdf->Write(5, 'RC Linderos: $'.formatNumber($row_Recordset2['integral_comercio_rc_lind'], 2));
