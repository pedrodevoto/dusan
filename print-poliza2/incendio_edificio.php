<?php
$sql = sprintf("SELECT *  FROM incendio_edificio WHERE poliza_id=%s", $row['poliza_id']);
...
switch(substr($_GET['type'], 0, 2)) {
	...
	$pdf->wwrite(13, 97, sprintf('Dirección: %s %s', $row2['incendio_edificio_domicilio_calle'], $row2['combinado_familiar_domicilio_nro']));

	$pdf->wwrite(100, 97, sprintf('Piso/Dpto: %s %s', $row2['incendio_edificio_domicilio_piso'], $row2['combinado_familiar_domicilio_dpto']));
	
	$pdf->wwrite(130, 97, sprintf('Localidad: %s', $row2['incendio_edificio_domicilio_localidad']));
	$pdf->wwrite(180, 97, sprintf('CP: %s', $row2['incendio_edificio_domicilio_cp']));
	
	
	$pdf->wwrite(13, 102, sprintf('Barrio cerrado/country: %s', $row2['incendio_edificio_country']));
	$pdf->wwrite(100, 102, sprintf('Lote: %s', $row2['incendio_edificio_lote']));
	
	$pdf->wwrite(13, 107, sprintf('Incendio Edificio:                      $%s', formatNumber($row2['incendio_edificio_inc_edif'], 2)));
	$pdf->wwrite(13, 112, sprintf('Incendio Mobiliario:                  $%s', formatNumber($row2['incendio_edificio_inc_mob'], 2)));
	$pdf->wwrite(13, 117, sprintf('RC por Incendio:                      $%s', formatNumber($row2['incendio_edificio_rc_inc'], 2)));
	$pdf->wwrite(13, 122, sprintf('Valor tasado de la propiedad:  $%s', formatNumber($row2['incendio_edificio_valor_tasado'], 2)));
	...
	$pdf->wwrite(13, 92, sprintf('Dirección: %s %s', $row2['incendio_edificio_domicilio_calle'], $row2['combinado_familiar_domicilio_nro']));

	$pdf->wwrite(100, 92, sprintf('Piso/Dpto: %s %s', $row2['incendio_edificio_domicilio_piso'], $row2['combinado_familiar_domicilio_dpto']));
	
	$pdf->wwrite(130, 92, sprintf('Localidad: %s', $row2['incendio_edificio_domicilio_localidad']));
	$pdf->wwrite(180, 92, sprintf('CP: %s', $row2['incendio_edificio_domicilio_cp']));
