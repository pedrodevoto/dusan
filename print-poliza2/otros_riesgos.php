<?php
$sql = sprintf("SELECT *  FROM otros_riesgos WHERE poliza_id=%s", $row['poliza_id']);
...
switch(substr($_GET['type'], 0, 2)) {
	...
	$pdf->wwrite(146, 60, sprintf('Tipo de seguro: OTROS RIESGOS'), 8);
	...
	$pdf->wwrite(13, 97, sprintf('Riesgo asegurado: %s', $row2['otros_riesgos_riesgo']));
	$pdf->wwrite(13, 102, sprintf('Datos del riesgo: %s', $row2['otros_riesgos_datos_riesgo']));
	$pdf->wwrite(13, 107, sprintf('Detalle del riesgo: %s', $row2['otros_riesgos_detalle_riesgo']));
