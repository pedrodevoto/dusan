		...
		case 'automotor':		
			...
			if ($totalRows_Recordset2 === 0) {				
				...
				$updateSQL = sprintf("UPDATE automotor 
										SET automotor_marca_id=%s, modelo=UPPER(TRIM(%s)), castigado=%s, patente=UPPER(TRIM(%s)), automotor_tipo_id=%s, uso=%s, ano=%s, automotor_carroceria_id=%s, combustible=%s, 0km=%s, importado=%s, nro_motor=UPPER(TRIM(%s)), nro_chasis=UPPER(TRIM(%s)), chapa=%s, pintura=%s, tipo_pintura=%s, tapizado=%s, color=UPPER(TRIM(%s)), accesorios=%s, zona_riesgo_id=%s, prendado=%s, acreedor_rs=UPPER(TRIM(%s)), acreedor_cuit=UPPER(TRIM(%s)), infoauto=%s, observaciones=%s, alarma=%s, corta_corriente=%s, corta_nafta=%s, traba_volante=%s, matafuego=%s, tuercas=%s, equipo_rastreo=%s, equipo_rastreo_pedido_id=%s, equipo_rastreo_id=%s, micro_grabado=%s, cupon_vintrak=%s, cupon_vintrak_fecha=%s, antena=%s, estereo=%s, parlantes=%s, aire=%s, cristales_electricos=%s, faros_adicionales=%s, cierre_sincro=%s, techo_corredizo=%s, direccion_hidraulica=%s, frenos_abs=%s, airbag=%s, cristales_tonalizados=%s, gps=%s, 
