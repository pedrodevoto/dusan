	...
	switch ($_GET['mostrar']) {
		...
		case 'modelos':
			...
			if(!empty($_GET['marca_id'])){	
				$query_Recordset1_where .= sprintf(" WHERE automotor_marca_id = %s",GetSQLValueString($_GET['marca_id'], "int"));
			}
			else {
		...
		case 'versiones':
			...
			if(!empty($_GET['modelo_id'])){	
				$query_Recordset1_where .= sprintf(" WHERE automotor_modelo_id = %s",GetSQLValueString($_GET['modelo_id'], "int"));
			}
			else {
				$query_Recordset1_where = " WHERE 1=2";
			}
...
<?php
	...
	switch ($action) {
		...
		case "view":
			...
			$aColumns = array('id', 'tipo_registro', 'nombre', ' ');
