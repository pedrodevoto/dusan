...
<?php
	...
	if(isset($_GET['fotos']) && $_GET['fotos']!=""){	
		$query_Recordset1_having .= sprintf(" AND COUNT(poliza_foto_id) %s 0", $_GET['fotos']=='1'?'>':'=');
	}
	// Filter by: castigado
	...
	// Filter by: cobertura_tipo_id
	if(isset($_GET['cobertura_tipo_id']) && $_GET['cobertura_tipo_id']!=""){	
		$query_Recordset1_where .= sprintf(" AND automotor.cobertura_tipo_id = %s",GetSQLValueString($_GET['cobertura_tipo_id'], "int"));
	}
