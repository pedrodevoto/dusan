...
<?php
	...
	// Require DB functions6
	require_once('inc/db_functions.php');		
?>
<?php

	// GENERATE MAIN QUERY (WITHOUT SELECT STATEMENT)
	$query_Recordset1_fields = " poliza.poliza_id, poliza.cliente_id as cliente_id, automotor.automotor_id as automotor_id, poliza_numero, IF(poliza_flota=1, 'FLOTA', CONCAT(IF(automotor_carroceria_id=17, '101', ''), patente_0, patente_1)) as patente, seguro_nombre, sucursal_nombre, CONCAT(productor_nombre, ' (', productor_seguro_codigo, ')') as productor_nombre, IF(cliente_tipo_persona=1, TRIM(CONCAT(IFNULL(cliente_apellido, ''), ' ', IFNULL(cliente_nombre, ''))), cliente_razon_social) as cliente_nombre, CONCAT(poliza_vigencia, IF(poliza_vigencia='Otra', CONCAT(' (', poliza_vigencia_dias, ')'), '')) as poliza_vigencia, DATE_FORMAT(poliza_validez_desde, '%d/%m/%y') as poliza_validez_desde_formateada, DATE_FORMAT(poliza_validez_hasta, '%d/%m/%y') as poliza_validez_hasta, IF(poliza_entregada=1, 'Entregada', IF(poliza_correo=1, 'Enviada', IF(poliza_email=1, 'Enviada', IF(poliza_fecha_recepcion IS NOT NULL, 'Sí', 'No')))) as poliza_estado_entrega, IF(COUNT(endoso_id) > 0, 'ANULADA', poliza_estado_nombre) as poliza_estado_nombre, IF(automotor_carroceria_id=17, 'N/A', IF(COUNT(automotor_foto_id) > 0, 'Sí', IF(COUNT(automotor_cert_rodamiento_archivo_id)>0, 'Sí', 'No'))) as automotor_fotos, IF(poliza_medio_pago = 'Directo', IF(COUNT(IF(cuota_vencimiento <= DATE(NOW()) AND cuota_estado_id = 1, IF(endoso_tipo_grupo_id=1 AND DATE(endoso_fecha_pedido)<=DATE(cuota_vencimiento),NULL, 1), NULL))=0, 'Sí', 'No'), IF(poliza_medio_pago='Cuponera' OR poliza_medio_pago LIKE '%cupon%', 'Cup', IF(poliza_medio_pago='Débito Bancario', 'DC', 'TC'))) AS poliza_al_dia, IF(poliza_medio_pago = 'Directo', GROUP_CONCAT(IF(cuota_vencimiento <= DATE(NOW()) AND cuota_estado_id = 1, CONCAT('Cuota número ', cuota_nro, ' (Período: ', DATE_FORMAT(cuota_periodo, '%m/%y'), ', venc: ', IF(DATE(cuota_vencimiento) = DATE(NOW()), 'hoy', DATE_FORMAT(cuota_vencimiento, '%d/%m/%y')), ')'), NULL) SEPARATOR '\n'), '') AS poliza_al_dia_detalle, poliza_flota, SUM(IF(cuota_vencimiento = DATE_ADD(DATE(NOW()), INTERVAL 1 DAY) AND cuota_estado_id=1, 1, 0)) as vence_manana";
	$query_Recordset1_tables = " FROM poliza LEFT JOIN (productor_seguro, seguro, productor, cliente) ON (poliza.productor_seguro_id=productor_seguro.productor_seguro_id AND productor_seguro.seguro_id=seguro.seguro_id AND productor_seguro.productor_id=productor.productor_id AND poliza.cliente_id=cliente.cliente_id) JOIN sucursal ON poliza.sucursal_id = sucursal.sucursal_id LEFT JOIN cuota ON cuota.poliza_id = poliza.poliza_id JOIN subtipo_poliza ON subtipo_poliza.subtipo_poliza_id = poliza.subtipo_poliza_id LEFT JOIN automotor ON poliza.poliza_id=automotor.poliza_id LEFT JOIN automotor_foto ON automotor.automotor_id = automotor_foto.automotor_id JOIN poliza_estado on poliza_estado.poliza_estado_id = poliza.poliza_estado_id LEFT JOIN (endoso, endoso_tipo) ON (poliza.poliza_id = endoso.poliza_id AND endoso.endoso_tipo_id = endoso_tipo.endoso_tipo_id AND endoso_tipo_grupo_id = 1) LEFT JOIN seguro_cobertura_tipo ON automotor.seguro_cobertura_tipo_id = seguro_cobertura_tipo.seguro_cobertura_tipo_id LEFT JOIN automotor_cert_rodamiento_archivo ON automotor_cert_rodamiento_archivo.automotor_id = automotor.automotor_id";
	$query_Recordset1_where = " WHERE poliza_archivada IS NULL AND poliza.subtipo_poliza_id = 6";
	...
	if(isset($_GET['patente']) && $_GET['patente']!=""){	
		$filter_where .= sprintf(" AND CONCAT(IF(automotor_carroceria_id=17, '101', ''), patente_0, patente_1) LIKE %s",GetSQLValueString('%' . $_GET['patente'] . '%', "text"));
	}
	...
	if(isset($_GET['productor_id']) && $_GET['productor_id']!=""){	
		$filter_where .= sprintf(" AND productor_seguro.productor_id = %s",GetSQLValueString($_GET['productor_id'], "int"));
	}
	...
	if(isset($_GET['fotos']) && $_GET['fotos']!=""){	
		$filter_having .= sprintf(" AND automotor_fotos IN %s ", $_GET['fotos']=='1'?'("Sí", "N/A")':'("No")');
	}
	// Filter by: castigado
	if(isset($_GET['castigado']) && $_GET['castigado']!=""){	
		$filter_where .= sprintf(" AND automotor.castigado = %s",GetSQLValueString($_GET['castigado'], "int"));
	}
	// Filter by: micro_grabado
	if(isset($_GET['micro_grabado']) && $_GET['micro_grabado']!=""){	
		$filter_where .= sprintf(" AND automotor.micro_grabado = %s",GetSQLValueString($_GET['micro_grabado'], "int"));
	}
	// Filter by: seguro_cobertura_tipo_nombre
	if(isset($_GET['seguro_cobertura_tipo_nombre']) && $_GET['seguro_cobertura_tipo_nombre']!=""){	
		$filter_where .= sprintf(" AND seguro_cobertura_tipo_nombre = %s",GetSQLValueString($_GET['seguro_cobertura_tipo_nombre'], "text"));
	}
...
<?php
	...
	switch ($action) {
		...
		case "view":
			...
			$aColumns = array('poliza_id', 'cliente_id', 'automotor_id', 'poliza_numero', 'patente', 'seguro_nombre', 'sucursal_nombre', 'productor_nombre', 'cliente_nombre', 'poliza_vigencia', 'poliza_validez_desde_formateada', 'poliza_validez_hasta', 'poliza_estado_entrega', 'poliza_estado_nombre', 'automotor_fotos', 'poliza_al_dia', 'poliza_al_dia_detalle', 'poliza_flota', ' ');
