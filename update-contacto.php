...
<?php
	if ((isset($_POST["box-contacto_id"])) && ($_POST["box-contacto_id"] != "")) {		
		...
		// Update
		$updateSQL = sprintf("UPDATE contacto SET contacto_tipo=%s, contacto_domicilio=UPPER(TRIM(%s)), contacto_nro=UPPER(TRIM(%s)), contacto_piso=UPPER(TRIM(%s)), contacto_dpto=UPPER(TRIM(%s)), localidad_id=%s, contacto_country=UPPER(TRIM(%s)), contacto_lote=UPPER(TRIM(%s)), contacto_telefono1=UPPER(TRIM(%s)), contacto_telefono2=UPPER(TRIM(%s)), contacto_telefono2_compania=%s, contacto_telefono_laboral=UPPER(TRIM(%s)), contacto_telefono_alt=UPPER(TRIM(%s)), contacto_observaciones=UPPER(TRIM(%s)) WHERE contacto_id=%s LIMIT 1",
