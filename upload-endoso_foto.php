...
if ($_FILES["box-endoso_foto"]["error"] == 0 and isset($_POST['endoso_id'])){
	if ($_FILES["box-endoso_foto"]["size"] > return_bytes(ini_get('upload_max_filesize'))) {
		die("Error: no se puede subir archivos de más de ".ini_get('upload_max_filesize')." de tamaño.");
	}
	$extension = strtolower(strrchr($_FILES["box-endoso_foto"]["name"], '.'));
	if (!in_array($extension, array(".jpg", ".jpeg", ".png"))) {
		die("Error: subir solamente fotos con extensión .jpg, .jpeg o .png");
	}
	$filename = "fotos/" . str_replace('/', '_', $_POST['endoso_id']) . time();
	if (move_uploaded_file($_FILES["box-endoso_foto"]["tmp_name"], $filename . $extension )) {
		...
		if (!imagepng($thumb, $thumb_filename, 0)) {
			...
			$insertSQL = sprintf("INSERT INTO endoso_foto (endoso_id, endoso_foto_url, endoso_foto_thumb_url, endoso_foto_width, endoso_foto_height) VALUES (%s, %s, %s, %s, %s)",
						GetSQLValueString($_POST['endoso_id'], "int"),
